...
it's very very hard and not fun to do), then you are not using an code
...
hindered by your code editor throughout this journey?
...
Pope](http://www.vim.org/account/profile.php?user_id=9012) Bus (if Tim
Pope dies, I and many Vim users are screwed). I just didn't want to
...
have 30+ terraflops of computing power up there, and response times
...
## But Every Editor Can be Extented ##
...
how you would go about doing the challenge in next section?**

Additionally, think about the community that uses the
editor. What percentage of people that use the editor know how to
extend it? With regards to Emacs, most know how to extend it, and
enjoy doing so (I'm keeping a close eye on Atom these days for this
every reason, the ubiquity of JavasScript may lead to a much larger
community of people who are knowledgable in editor extension).
...
## 2. Your Editor Should Let You Execute an Editor Function Arbitrarily ##
...
- Create a temporary buffer, type a url, http/method, and json
  payload, run a keyboard shortcut to peform an http request. Get the
...
- Conditionall load plugins based on file type (more specifically `major-mode`).
